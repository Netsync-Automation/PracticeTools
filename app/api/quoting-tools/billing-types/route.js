...
export async function POST(request) {
  try {
    ...
    if (!name?.trim()) {
      return NextResponse.json({ error: 'Billing type name is required' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ success: true, billingType: item });
    } catch (error) {
    ...
    console.error('Billing types POST error:', error);
    return NextResponse.json({ error: 'Failed to save billing type' }, { status: 500 });
  }
}