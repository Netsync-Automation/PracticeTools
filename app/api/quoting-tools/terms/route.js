...
export async function POST(request) {
  try {
    const { months, createdBy, createdByName } = await request.json();
    
    if (!months || isNaN(months) || months < 1) {
      return NextResponse.json({ error: 'Valid term length in months is required' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ success: true, term: item });
    } catch (error) {
