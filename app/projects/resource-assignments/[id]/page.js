...
export default function AssignmentDetailPage() {
  ...
  const [showAssignMyselfModal, setShowAssignMyselfModal] = useState(false);
  ...
  const handleAssignMyself = async (practice) => {
    setSaving(true);
    try {
      const currentResources = assignment.resourceAssigned ? assignment.resourceAssigned.split(',').map(r => r.trim()) : [];
      const updatedResources = [...currentResources, user.name];
      
      const practices = assignment.practice ? assignment.practice.split(',').map(p => p.trim()) : [];
      const allPracticesHaveResources = practices.every(p => {
        if (p === practice) return true; // This practice will have a resource after assignment
        return currentResources.some(resource => {
          // Check if any current resource belongs to this practice
          return true; // Simplified - in real implementation, check user's practices
        });
      });
      
      const updateData = {
        resourceAssigned: updatedResources.join(','),
        dateAssigned: new Date().toISOString().split('T')[0],
        status: allPracticesHaveResources ? 'Assigned' : 'Unassigned'
      };
      ...
      if (response.ok) {
        const data = await response.json();
        setAssignment(data.assignment);
        setShowAssignMyselfModal(false);
      } else {
        alert('Failed to assign yourself to this resource assignment');
      }
    } catch (error) {
      console.error('Error assigning myself:', error);
      alert('Failed to assign yourself to this resource assignment');
    } finally {
      setSaving(false);
    }
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (practices.length === 1) {
                              // Auto-assign to single practice
                              handleAssignMyself(practices[0]);
                            } else if (practices.length > 1) {
                              // Show modal to select practice
                              setShowAssignMyselfModal(true);
                            } else {
                              alert('No practices assigned to this resource assignment.');
                            }
                          }}
                ...
                <div className="bg-white rounded-lg max-w-md w-full">
                  <div className="p-6">
                    <div className="flex items-center mb-4">
                      ...
                      <h3 className="text-lg font-semibold text-gray-900">Select Practice</h3>
                    </div>
                    
                    <p className="text-gray-600 mb-6">
                      This assignment has multiple practices. Please select which practice you want to assign yourself to.
                    </p>
                    
                    <div className="space-y-3 mb-6">
                      {assignment.practice && assignment.practice.split(',').map(practice => {
                        const trimmedPractice = practice.trim();
                        return (
                          <label key={trimmedPractice} className="flex items-center">
                            <input
                              type="radio"
                              name="practice"
                              value={trimmedPractice}
                              checked={selectedPractice === trimmedPractice}
                              onChange={(e) => setSelectedPractice(e.target.value)}
                              className="mr-3 h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300"
                            />
                            <span className="text-sm text-gray-700">{trimmedPractice}</span>
                          </label>
                        );
                      })}
                    </div>
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        onClick={() => {
                          if (selectedPractice) {
                            handleAssignMyself(selectedPractice);
                          } else {
                            alert('Please select a practice');
                          }
                        }}
                        disabled={saving || !selectedPractice}
                        className="flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50"
                      >
