...
export class DynamoDBService {
  ...
  async toggleTrainingCertSignUp(trainingCertId, userEmail, userName, forceAction = null) {
    try {
      ...
      if (forceAction === 'add') {
        if (existingSignUpIndex === -1) {
          ...
          return { success: true, action: 'already_signed_up', signedUp: true };
        }
      ...
      return { success: true, action, signedUp };
    } catch (error) {
  ...
  async addTrainingCert(practice, type, vendor, name, code, level, trainingType, prerequisites, examsRequired, examCost, quantityNeeded, incentive, notes, createdBy) {
    ...
    console.log('[ADD-TRAINING-CERT] Creating with createdBy:', createdBy);
    ...
    if (createdBy) {
      try {
        ...
        console.log('[ADD-TRAINING-CERT] Could not lookup user name for:', createdBy);
      }
    }
    ...
    console.log('[ADD-TRAINING-CERT] DynamoDB item to be created:', JSON.stringify(command.input.Item, null, 2));
    
    try {
      await this.client.send(command);
      console.log('[ADD-TRAINING-CERT] Successfully created training cert with ID:', id);
      return id;
    } catch (error) {
  ...
  async getAllTrainingCerts() {
    ...
    try {
      const result = await this.client.send(command);
      console.log('[GET-ALL-DEBUG] Raw DynamoDB item for first entry:', JSON.stringify(result.Items?.[0], null, 2));
      if (result.Items?.[0]) {
        console.log('[GET-ALL-DEBUG] updated_by field:', result.Items[0].updated_by?.S);
        console.log('[GET-ALL-DEBUG] updated_by_name field:', result.Items[0].updated_by_name?.S);
      }
      
      // Process items with async user lookups
      const trainingCerts = [];
      for (const item of result.Items || []) {
        const formattedItem = await this.formatTrainingCertItem(item);
        trainingCerts.push(formattedItem);
      }
  ...
  async updateTrainingCert(id, updates) {
    try {
      console.log('[DB-UPDATE-TRAINING-CERT] Called with ID:', id);
      console.log('[DB-UPDATE-TRAINING-CERT] Updates object:', updates);
      ...
      Object.keys(updates).forEach(key => {
        if (key !== 'id' && key !== 'created_by' && key !== 'created_by_name' && updates[key] !== undefined && updates[key] !== null) {
          console.log('[DB-UPDATE-TRAINING-CERT] Processing field:', key, '=', updates[key]);
          if (key === 'updated_by') {
            ...
            console.log('[DB-UPDATE-TRAINING-CERT] Added updated_by_name field');
      ...
      console.log('[DB-UPDATE-TRAINING-CERT] Final update expressions:', updateExpressions);
      console.log('[DB-UPDATE-TRAINING-CERT] Final attribute values:', attributeValues);
      ...
      console.log('[DB-UPDATE-TRAINING-CERT] DynamoDB command:', JSON.stringify(command.input, null, 2));
      
      await this.client.send(command);
      console.log('[DB-UPDATE-TRAINING-CERT] Update successful');
      return true;
    } catch (error) {
  ...
  async completeTrainingCert(trainingCertId, userEmail, userName, certificateFile = null, notes = '') {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
      ...
      if (userSignUpIndex === -1) {
        ...
        const newSignUp = {
          ...
          certificateUrl: certificateFile ? `/uploads/certificates/${certificateFile.name}` : null,
  ...
  async getAllTrainingCerts() {
    ...
    try {
      ...
      return (result.Items || []).map(item => ({
        ...
        incentive: item.incentive?.S || '',
        notes: item.notes?.S || '',
        signUps: item.sign_ups ? JSON.parse(item.sign_ups.S) : [],
