...
export class DynamoDBService {
  ...
  async getCompanies(practiceGroupId, contactType) {
    try {
      const command = new ScanCommand({
        ...
        FilterExpression: 'practice_group_id = :practiceGroupId AND contact_type = :contactType',
        ExpressionAttributeValues: {
          ':practiceGroupId': { S: practiceGroupId },
      ...
      return (result.Items || []).map(item => ({
        ...
        tier: item.tier?.S || '',
        technology: item.technology?.S || '',
        solutionType: item.solution_type?.S || '',
    ...
    try {
      ...
      const command = new PutItemCommand({
        ...
        Item: {
          ...
          tier: { S: company.tier },
          technology: { S: company.technology },
          solution_type: { S: company.solutionType },
          website: { S: company.website },
          practice_group_id: { S: company.practiceGroupId },
          contact_type: { S: company.contactType },
          date_added: { S: company.dateAdded || new Date().toISOString() },
    ...
    try {
      ...
      const command = new PutItemCommand({
        ...
        Item: {
          ...
          date_added: { S: contact.dateAdded || new Date().toISOString() },
  ...
  async addSaAssignment(practice, status, opportunityId, requestDate, eta, customerName, opportunityName, region, am, saAssigned, dateAssigned, notes, attachments = [], notificationUsers = [], scoopUrl = '', isr = '', submittedBy = '') {
    ...
    // Process AM and ISR users and get their emails if running server-side
    let amEmail = '', isrEmail = '', submittedByEmail = '';
    if (typeof window === 'undefined') {
      try {
        const { processAmIsrUsers } = await import('./user-manager.js');
        const amIsrResult = await processAmIsrUsers(am, isr);
        
        // Get emails from processed users
        if (amIsrResult.amUser) {
          amEmail = amIsrResult.amUser.email;
        }
        if (amIsrResult.isrUser) {
          isrEmail = amIsrResult.isrUser.email;
        }
        
        // Get submitted by email
        if (submittedBy) {
        ...
        if (amIsrResult.amUser || amIsrResult.isrUser) {
          logger.info('Processed AM/ISR users during SA assignment creation', {
            amUserExists: !!amIsrResult.amUser,
            isrUserExists: !!amIsrResult.isrUser,
            amCreated: amIsrResult.amUser?.created_from === 'SYSTEM',
            isrCreated: amIsrResult.isrUser?.created_from === 'SYSTEM'
          });
        }
      } catch (error) {
        logger.error('Error processing AM/ISR users in addSaAssignment', { error: error.message });
