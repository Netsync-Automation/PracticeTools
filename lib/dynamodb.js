...
export class DynamoDBService {
  ...
  async toggleTrainingCertSignUp(trainingCertId, userEmail, userName, forceAction = null, iterations = 1) {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        logger.error('Training cert not found for sign-up', { trainingCertId });
        return { success: false, error: 'Training cert not found' };
      }

      // Parse existing sign-ups (use signUps from formatted object)
      const existingSignUps = trainingCert.signUps || [];
      
      // Check if user already signed up
      const existingSignUpIndex = existingSignUps.findIndex(signup => signup.email === userEmail);
      let updatedSignUps;
      let action;
      let signedUp;
      ...
      if (forceAction === 'add') {
        if (existingSignUpIndex === -1) {
          ...
          updatedSignUps = [...existingSignUps, newSignUp];
          action = 'added';
          signedUp = true;
        } else {
          // Update existing signup with additional iterations if provided
          const existingSignUp = existingSignUps[existingSignUpIndex];
          updatedSignUps = [...existingSignUps];
          updatedSignUps[existingSignUpIndex] = {
            ...existingSignUp,
            iterations: (existingSignUp.iterations || 1) + iterations
          };
          action = 'updated';
          signedUp = true;
        }
        ...
        if (existingSignUpIndex !== -1) {
          updatedSignUps = existingSignUps.filter(signup => signup.email !== userEmail);
          action = 'removed';
          signedUp = false;
        } else {
          return { success: true, action: 'not_signed_up', signedUp: false };
        }
      } else {
        // Default toggle behavior
        if (existingSignUpIndex !== -1) {
          // User is signed up - remove them
          updatedSignUps = existingSignUps.filter(signup => signup.email !== userEmail);
          action = 'removed';
          signedUp = false;
          logger.info('Training cert sign-up removed', { trainingCertId, userEmail, userName });
        } else {
          // User is not signed up - add them
          const newSignUp = {
            email: userEmail,
            name: userName,
            signedUpAt: new Date().toISOString(),
          ...
          action = 'added';
          signedUp = true;
          logger.info('Training cert sign-up added', { trainingCertId, userEmail, userName });
        }
      }

      // Update training cert with new sign-up list
      const command = new UpdateItemCommand({
        TableName: TABLES.TRAINING_CERTS,
        Key: { id: { S: trainingCertId } },
        UpdateExpression: 'SET sign_ups = :signUps, updated_at = :updatedAt',
        ExpressionAttributeValues: {
          ':signUps': { S: JSON.stringify(updatedSignUps) },
          ':updatedAt': { S: new Date().toISOString() }
        }
      });

      await this.client.send(command);
      const userSignup = updatedSignUps.find(signup => signup.email === userEmail);
      return { 
        success: true, 
        action, 
        signedUp,
        iterations: userSignup?.iterations || 0,
        completedIterations: userSignup?.completedIterations || 0
      };
    } catch (error) {
      logger.error('DynamoDB toggleTrainingCertSignUp error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async addTrainingCert(practice, type, vendor, name, code, level, trainingType, prerequisites, examsRequired, examCost, quantityNeeded, incentive, notes, createdBy) {
    const id = uuidv4();
    const timestamp = new Date().toISOString();
    
    // Look up user name for the creator
    let createdByName = '';
    if (createdBy) {
      try {
    ...
    const command = new PutItemCommand({
      ...
      Item: {
        ...
        quantity_needed: { S: quantityNeeded || '' },
        incentive: { S: incentive || '' },
        notes: { S: notes || '' },
        sign_ups: { S: '[]' },
        updated_by: { S: createdBy || '' },
        updated_by_name: { S: createdByName },
    ...
    try {
      ...
      if (error.name === 'ResourceNotFoundException') {
        ...
        return this.addTrainingCert(practice, type, vendor, name, code, level, trainingType, prerequisites, examsRequired, examCost, quantityNeeded, incentive, notes, createdBy);
      }
  ...
  async getTrainingCertById(id) {
    ...
    try {
      ...
      return result.Item ? await this.formatTrainingCertItem(result.Item) : null;
    } catch (error) {
      logger.error('DynamoDB getTrainingCertById error', { error: error.message });
      return null;
    }
  }
  ...
  async getAllTrainingCerts() {
    ...
    try {
      ...
      const trainingCerts = await Promise.all(
        (result.Items || []).map(item => this.formatTrainingCertItem(item))
      );
      
      return trainingCerts.sort((a, b) => new Date(b.updated_at || b.created_at) - new Date(a.updated_at || a.created_at));
    } catch (error) {
      if (error.name === 'ResourceNotFoundException') {
        await this.createTrainingCertsTable();
        return [];
  ...
  async updateTrainingCert(id, updates) {
    try {
      if (!id || typeof id !== 'string') {
        logger.error('Invalid ID provided for updateTrainingCert', { id });
        return { success: false, error: 'Invalid ID provided' };
      }
      
      if (!updates || typeof updates !== 'object') {
        logger.error('Invalid updates object for updateTrainingCert', { updates });
      ...
      // DSR: Check if quantityNeeded is being reduced and handle data cleanup
      if (updates.quantityNeeded !== undefined) {
        const currentEntry = await this.getTrainingCertById(id);
        if (currentEntry) {
          const newQuantity = parseInt(updates.quantityNeeded) || 0;
          const currentQuantity = parseInt(currentEntry.quantityNeeded) || 0;
          
          if (newQuantity < currentQuantity) {
            const signUps = currentEntry.signUps || [];
            
            // Find signups that will be affected by the reduction
            const affectedUsers = [];
            let updatedSignUps = [...signUps];
            
            for (let i = 0; i < signUps.length; i++) {
              const signup = signUps[i];
              const userIterations = signup.iterations || 1;
              const completedIterations = signup.completedIterations || 0;
              
              // Calculate how many iterations this user should have after reduction
              let newUserIterations = userIterations;
              let newCompletedIterations = completedIterations;
              
              // If this user's iterations exceed the new limit, reduce them
              if (userIterations > newQuantity) {
                newUserIterations = Math.max(1, newQuantity); // Ensure at least 1 iteration
                newCompletedIterations = Math.min(completedIterations, newUserIterations);
                
                // Track affected user
                affectedUsers.push({
                  email: signup.email,
                  name: signup.name,
                  oldIterations: userIterations,
                  newIterations: newUserIterations,
                  oldCompleted: completedIterations,
                  newCompleted: newCompletedIterations,
                  removedIterations: Math.max(0, completedIterations - newCompletedIterations)
                });
                
                // Remove excess iteration certificates
                let updatedIterationCerts = signup.iterationCertificates || [];
                if (newCompletedIterations < completedIterations) {
                  updatedIterationCerts = updatedIterationCerts.filter(cert => cert.iteration <= newCompletedIterations);
                }
                
                // Update the signup
                updatedSignUps[i] = {
                  ...signup,
                  iterations: newUserIterations,
                  completedIterations: newCompletedIterations,
                  completed: newCompletedIterations >= newUserIterations,
                  iterationCertificates: updatedIterationCerts,
            ...
            if (affectedUsers.length > 0) {
              warning = `Reducing iterations from ${currentQuantity} to ${newQuantity} affected ${affectedUsers.length} user signup(s). Excess completed iterations and certificates have been removed.`;
              affectedSignups = affectedUsers;
              
              // Update the signups in the database
              const updateSignupsCommand = new UpdateItemCommand({
                TableName: TABLES.TRAINING_CERTS,
                Key: { id: { S: id } },
                UpdateExpression: 'SET sign_ups = :signUps, updated_at = :updatedAt',
                ExpressionAttributeValues: {
              ...
              await this.client.send(updateSignupsCommand);
              
              logger.info('Training cert iteration reduction cleanup performed', {
                trainingCertId: id,
                currentQuantity,
                newQuantity,
                affectedUsers: affectedUsers.length,
                cleanupDetails: affectedUsers
              });
              ...
              try {
                ...
                await sendSSENotification('all', {
                  type: 'training_cert_updated',
                  trainingCertId: id,
                  action: 'iterations_reduced_cleanup',
                  affectedUsers: affectedUsers.length,
                  timestamp: Date.now()
                });
              } catch (sseError) {
                logger.error('Failed to send SSE notification for iteration reduction cleanup', { error: sseError.message });
              }
            }
          }
        }
      ...
      // Process all valid update fields (but never update created_by or created_by_name after creation)
      Object.keys(updates).forEach(key => {
        if (key !== 'id' && key !== 'created_by' && key !== 'created_by_name' && updates[key] !== undefined && updates[key] !== null) {
          if (key === 'updated_by') {
            updateExpressions.push(`#updated_by = :updated_by`);
            attributeNames['#updated_by'] = 'updated_by';
            attributeValues[':updated_by'] = { S: String(updates[key] || '') };
          } else if (key === 'updated_by_name') {
            updateExpressions.push(`#updated_by_name = :updated_by_name`);
            attributeNames['#updated_by_name'] = 'updated_by_name';
            attributeValues[':updated_by_name'] = { S: String(updates[key] || '') };
          } else if (key === 'quantityNeeded') {
            updateExpressions.push(`#quantity_needed = :quantity_needed`);
            attributeNames['#quantity_needed'] = 'quantity_needed';
            attributeValues[':quantity_needed'] = { S: String(updates[key] || '') };
          } else if (key === 'examCost') {
            updateExpressions.push(`#exam_cost = :exam_cost`);
            attributeNames['#exam_cost'] = 'exam_cost';
            attributeValues[':exam_cost'] = { S: String(updates[key] || '') };
          } else if (key === 'examsRequired') {
            updateExpressions.push(`#exams_required = :exams_required`);
            attributeNames['#exams_required'] = 'exams_required';
            attributeValues[':exams_required'] = { S: String(updates[key] || '') };
          } else if (key === 'trainingType') {
            updateExpressions.push(`#training_type = :training_type`);
            attributeNames['#training_type'] = 'training_type';
            attributeValues[':training_type'] = { S: String(updates[key] || '') };
          } else {
            updateExpressions.push(`#${key} = :${key}`);
            attributeNames[`#${key}`] = key;
            attributeValues[`:${key}`] = { S: String(updates[key] || '') };
          }
        }
      });
      
      if (updateExpressions.length === 0) {
        return { success: true, warning, affectedSignups, cleanupPerformed };
      }
      
      // Add timestamp and ensure updated_at is always set
      const timestamp = new Date().toISOString();
      updateExpressions.push('#updated_at = :updated_at');
      attributeNames['#updated_at'] = 'updated_at';
      attributeValues[':updated_at'] = { S: timestamp };
      ...
      logger.error('DynamoDB updateTrainingCert error', { 
        error: error.message, 
        id, 
  ...
  async completeTrainingCertIterations(trainingCertId, userEmail, userName, completedIterations, certificateUrl = null, notes = '', iterationCertificates = []) {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        logger.error('Training cert not found for completion', { trainingCertId });
        return { success: false, error: 'Training cert not found' };
      }

      // Check if user is signed up
      const existingSignUps = trainingCert.signUps || [];
      const userSignUpIndex = existingSignUps.findIndex(signup => signup.email === userEmail);
      
      if (userSignUpIndex === -1) {
        logger.error('User not signed up for training cert', { trainingCertId, userEmail });
        return { success: false, error: 'User is not signed up for this training' };
      }

      const userSignUp = existingSignUps[userSignUpIndex];
      const currentCompleted = userSignUp.completedIterations || 0;
      const totalIterations = userSignUp.iterations || 1;
      const newCompletedTotal = Math.min(currentCompleted + completedIterations, totalIterations);
      
      // Process iteration certificates
      const existingIterationCerts = userSignUp.iterationCertificates || [];
      const updatedIterationCerts = [...existingIterationCerts];
      
      // Add new iteration certificates
      iterationCertificates.forEach(cert => {
        if (cert.certificateUrl || cert.notes) {
          updatedIterationCerts.push({
      ...
      // Update existing sign-up record with completion info
      const updatedSignUps = [...existingSignUps];
      updatedSignUps[userSignUpIndex] = {
        ...userSignUp,
        completedIterations: newCompletedTotal,
        completed: newCompletedTotal >= totalIterations,
        lastCompletedAt: new Date().toISOString(),
        certificateUrl: certificateUrl || userSignUp.certificateUrl, // Legacy single certificate
        completionNotes: notes || userSignUp.completionNotes,
        iterationCertificates: updatedIterationCerts
      };
      ...
      await this.client.send(command);
      logger.info('Training cert iterations completed', { trainingCertId, userEmail, userName, completedIterations, newCompletedTotal });
      return { 
        success: true, 
        action: 'completed', 
        signedUp: true, 
      ...
      logger.error('DynamoDB completeTrainingCertIterations error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async completeTrainingCert(trainingCertId, userEmail, userName, certificateUrl = null, notes = '') {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
      ...
      if (userSignUpIndex === -1) {
        // User not signed up - sign them up first, then mark as complete
        const newSignUp = {
          email: userEmail,
          name: userName,
          signedUpAt: new Date().toISOString(),
        ...
        await this.client.send(command);
        logger.info('Training cert signed up and completed', { trainingCertId, userEmail, userName });
        return { success: true, action: 'completed', signedUp: true, completed: true };
      } else {
        // Update existing sign-up record with completion info
        const updatedSignUps = [...existingSignUps];
        updatedSignUps[userSignUpIndex] = {
        ...
        await this.client.send(command);
        logger.info('Training cert completion recorded', { trainingCertId, userEmail, userName });
        return { success: true, action: 'completed', signedUp: true, completed: true };
      }
    } catch (error) {
      logger.error('DynamoDB completeTrainingCert error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async uncompleteTrainingCert(trainingCertId, userEmail, userName) {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        logger.error('Training cert not found for un-completion', { trainingCertId });
        return { success: false, error: 'Training cert not found' };
      }

      // Check if user is signed up and completed
      const existingSignUps = trainingCert.signUps || [];
      const userSignUpIndex = existingSignUps.findIndex(signup => signup.email === userEmail);
      ...
      if (!userSignUp.completed) {
        return { success: true, action: 'not_completed', signedUp: true, completed: false };
      }

      // Update the sign-up record to remove completion info but preserve iterations
      const updatedSignUps = [...existingSignUps];
      updatedSignUps[userSignUpIndex] = {
        email: userSignUp.email,
        name: userSignUp.name,
        signedUpAt: userSignUp.signedUpAt || new Date().toISOString(),
        iterations: userSignUp.iterations || 1,
        completedIterations: 0
        // Remove completed, completedAt, certificateUrl, completionNotes, iterationCertificates
      };
      ...
      await this.client.send(command);
      logger.info('Training cert un-completion recorded', { trainingCertId, userEmail, userName });
      return { success: true, action: 'uncompleted', signedUp: true, completed: false };
    } catch (error) {
      logger.error('DynamoDB uncompleteTrainingCert error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async editTrainingCertSignupIterations(trainingCertId, userEmail, newIterations) {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        logger.error('Training cert not found for signup edit', { trainingCertId });
        return { success: false, error: 'Training cert not found' };
      }
      ...
      // Check if reducing iterations would affect completed ones
      if (newIterations < completedIterations) {
        const iterationCerts = userSignUp.iterationCertificates || [];
        const affectedIterations = iterationCerts.filter(cert => cert.iteration > newIterations);
        return {
          success: false,
      ...
      updatedSignUps[userSignUpIndex] = {
        ...
        completed: completedIterations >= newIterations,
        lastEditedAt: new Date().toISOString()
      };
      ...
      await this.client.send(command);
      logger.info('Training cert signup iterations updated', { trainingCertId, userEmail, currentIterations, newIterations });
      return { 
        success: true, 
        action: 'iterations_updated',
        previousIterations: currentIterations,
        newIterations: newIterations
      };
    } catch (error) {
      logger.error('DynamoDB editTrainingCertSignupIterations error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async editTrainingCertSignupIterationsForce(trainingCertId, userEmail, newIterations) {
    try {
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        return { success: false, error: 'Training cert not found' };
      ...
      // DSR: Remove iteration certificates beyond new limit and log what's being removed
      let updatedIterationCerts = userSignUp.iterationCertificates || [];
      let removedCertificates = [];
      if (newIterations < completedIterations) {
        removedCertificates = updatedIterationCerts.filter(cert => cert.iteration > newIterations);
        updatedIterationCerts = updatedIterationCerts.filter(cert => cert.iteration <= newIterations);
        
        logger.info('Training cert iteration reduction - removing certificates', {
          trainingCertId,
          userEmail,
          removedCertificates: removedCertificates.length,
          removedIterations: removedCertificates.map(cert => cert.iteration)
        });
      }
      
      // Update completion status
      const newCompletedIterations = Math.min(completedIterations, newIterations);
      ...
      updatedSignUps[userSignUpIndex] = {
        ...
        iterations: newIterations,
        completedIterations: newCompletedIterations,
        completed: newCompletedIterations >= newIterations,
        iterationCertificates: updatedIterationCerts,
        lastEditedAt: new Date().toISOString()
      };
      ...
      try {
        ...
        await sendSSENotification('all', {
          type: 'training_cert_updated',
          trainingCertId: trainingCertId,
          userEmail: userEmail,
          action: 'iterations_reduced',
          newIterations: newIterations,
          removedIterations: Math.max(0, completedIterations - newIterations),
          timestamp: Date.now()
        });
      } catch (sseError) {
        logger.error('Failed to send SSE notification for iteration reduction', { error: sseError.message });
      }
      
      return { 
        success: true, 
        action: 'iterations_updated_forced',
        previousIterations: currentIterations,
        newIterations: newIterations,
        removedIterations: Math.max(0, completedIterations - newIterations),
        removedCertificates: removedCertificates.length
      ...
      logger.error('DynamoDB editTrainingCertSignupIterationsForce error', { error: error.message });
      return { success: false, error: error.message };
    }
  }

  async editTrainingCertIteration(trainingCertId, userEmail, iterationNumber, certificateUrl = null, notes = '') {
    try {
      // Get current training cert
      const trainingCert = await this.getTrainingCertById(trainingCertId);
      if (!trainingCert) {
        logger.error('Training cert not found for iteration edit', { trainingCertId });
        return { success: false, error: 'Training cert not found' };
      }
      ...
      const updatedIterationCerts = [...existingIterationCerts];
      const iterationIndex = updatedIterationCerts.findIndex(cert => cert.iteration === iterationNumber);
      
      if (iterationIndex !== -1) {
        // Update existing iteration
        updatedIterationCerts[iterationIndex] = {
          ...updatedIterationCerts[iterationIndex],
          certificateUrl: certificateUrl || updatedIterationCerts[iterationIndex].certificateUrl,
          notes: notes || updatedIterationCerts[iterationIndex].notes,
          lastEditedAt: new Date().toISOString()
        };
      } else {
        // Create new iteration entry if it doesn't exist
        updatedIterationCerts.push({
      ...
      await this.client.send(command);
      logger.info('Training cert iteration edited', { trainingCertId, userEmail, iterationNumber });
      return { success: true, action: 'iteration_edited' };
    } catch (error) {
      logger.error('DynamoDB editTrainingCertIteration error', { error: error.message, trainingCertId, userEmail });
      return { success: false, error: error.message };
    }
  }

  async getTrainingCertsSettings(practice) {
    try {
      if (!practice) {
        const command = new ScanCommand({
        ...
        (result.Items || []).forEach(item => {
          const practiceKey = item.practice?.S;
          if (practiceKey) {
            allSettings[practiceKey] = {
  ...
  async formatTrainingCertItem(item) {
    
    const signUps = JSON.parse(item.sign_ups?.S || '[]');
    const now = new Date().toISOString();
    const createdAt = item.created_at?.S || now;
    const updatedAt = item.updated_at?.S || createdAt;
    
    // Handle user information with proper fallbacks and user lookups
    let createdBy = 'Unknown User';
    let updatedBy = 'Unknown User';
    
    // Get user info from database fields
    const updatedByName = item.updated_by_name?.S || '';
    const updatedByEmail = item.updated_by?.S || '';
    const createdByEmail = item.created_by?.S || '';
    
    // For updatedBy: prioritize name, then lookup email, then fallback
    if (updatedByName) {
      updatedBy = updatedByName;
    } else if (updatedByEmail) {
      try {
        const user = await this.getUser(updatedByEmail);
        updatedBy = user?.name || updatedByEmail;
      } catch (error) {
        updatedBy = updatedByEmail;
      }
    }
    
    // For createdBy: prioritize created_by_name, then lookup created_by email, then fallback to updated info
    const createdByName = item.created_by_name?.S || '';
    if (createdByName) {
      createdBy = createdByName;
    } else if (createdByEmail) {
      try {
        const user = await this.getUser(createdByEmail);
        createdBy = user?.name || createdByEmail;
      } catch (error) {
        createdBy = createdByEmail;
      }
    } else {
      // Fallback to updated user info if no created_by
      createdBy = updatedBy;
    }
    ...
    return {
      ...
      quantityNeeded: item.quantity_needed?.S || '',
      incentive: item.incentive?.S || '',
      notes: item.notes?.S || '',
      signUps: signUps,
      createdBy: createdBy,
      updatedBy: updatedBy,
  ...
  async createOrUpdateUser(email, name, authMethod = 'saml', role = 'practice_member', password = null, createdFrom = 'manual', requirePasswordChange = false, isAdmin = false, practices = [], status = 'active', webexBotSource = null, region = null) {
    ...
    // Add WebEx bot sources array and primary source if provided
    if (webexBotSource) {
      item.webex_bot_sources = { S: JSON.stringify([webexBotSource]) };
      item.webex_bot_source = { S: webexBotSource }; // Primary team
    } else {
      item.webex_bot_sources = { S: JSON.stringify([]) };
    }
  ...
  async getUser(email) {
    ...
    // DSR: Try exact match first (for performance)
    const command = new GetItemCommand({
      TableName: TABLES.USERS,
      Key: { email: { S: email } }
    });
    ...
    try {
      ...
      // DSR: If exact match fails, try case-insensitive scan (fallback for existing data)
      console.log('[DB-USER-DEBUG] Exact match failed, trying case-insensitive scan...');
      const { normalizeEmail } = await import('./email-utils.js');
      ...
      const matchedItem = (scanResult.Items || []).find(item => {
        const itemEmail = item.email?.S || '';
        return normalizeEmail(itemEmail) === normalizedEmail;
      });
      
      if (matchedItem) {
        console.log('[DB-USER-DEBUG] Case-insensitive match found:', matchedItem.email?.S);
        return this.formatUserItem(matchedItem);
      }
  ...
  async getPracticeGroups() {
    try {
      ...
      return practiceManagers.map(manager => {
      ...
      console.error('[DB] getPracticeGroups - Error:', error.message);
      return [];
    }
  }
  ...
  async updateUser(email, updates) {
    ...
    if (updates.webex_bot_sources) {
      updateExpressions.push('webex_bot_sources = :webex_bot_sources');
      attributeValues[':webex_bot_sources'] = { S: JSON.stringify(updates.webex_bot_sources) };
    }
    ...
    return {
      ...
      webex_bot_sources: (() => {
        try {
          ...
          if (item.webex_bot_sources?.S) {
            return JSON.parse(item.webex_bot_sources.S);
          }
          // Backward compatibility: migrate old single source to array
          if (item.webex_bot_source?.S) {
            return [item.webex_bot_source.S];
          }
          return [];
        } catch {
          return [];
      ...
      webex_bot_source: item.webex_bot_source?.S || null, // Primary team
  ...
  async getPracticeWebexBot(practice) {
    console.log(`[DEBUG] getPracticeWebexBot called for practice: ${practice}`);
    try {
      const bots = await this.getWebexBots();
      console.log(`[DEBUG] Found ${bots.length} WebEx bots in database`);
      console.log(`[DEBUG] Available bots:`, bots.map(bot => ({ 
        id: bot.id, 
        name: bot.name, 
        practices: bot.practices,
        ssmPrefix: bot.ssmPrefix 
      })));
      
      const matchingBot = bots.find(bot => bot.practices && bot.practices.includes(practice));
      console.log(`[DEBUG] Matching bot for practice '${practice}':`, matchingBot ? {
        id: matchingBot.id,
        name: matchingBot.name,
        practices: matchingBot.practices,
        ssmPrefix: matchingBot.ssmPrefix
      } : null);
      ...
      // Get access token from SSM using the bot's ssmPrefix
      if (matchingBot.ssmPrefix) {
        try {
          const { getSecureParameter } = await import('./ssm-config.js');
          const env = process.env.ENVIRONMENT || 'dev';
          const ssmPrefix = env === 'prod' ? '/PracticeTools' : `/PracticeTools/${env}`;
          const accessTokenParam = `${ssmPrefix}/WEBEX_${matchingBot.ssmPrefix}_ACCESS_TOKEN`;
          
          console.log(`[DEBUG] Retrieving access token from SSM parameter: ${accessTokenParam}`);
          const accessToken = await getSecureParameter(accessTokenParam);
          
          if (accessToken) {
            console.log(`[DEBUG] Successfully retrieved access token for ${matchingBot.ssmPrefix}`);
            return {
              ...matchingBot,
              accessToken: accessToken
            };
          } else {
            console.log(`[DEBUG] No access token found for parameter: ${accessTokenParam}`);
            return null;
          }
        } catch (error) {
          console.error(`[DEBUG] Error retrieving access token for ${matchingBot.ssmPrefix}:`, error);
          return null;
        }
      }
  ...
  async getCardFollowers(cardKey) {
    try {
      // For now, return empty array as card following is not implemented yet
      // This function can be expanded when card following functionality is added
      return [];
    } catch (error) {
      logger.error('Error getting card followers:', error);
      return [];
    }
  }
  ...
  async updatePracticeAssignmentETA(practice) {
    // ETAs are now calculated in real-time, no need to update stored values
    return true;
  }
  ...
  async getPracticeETAs(practiceList = []) {
    try {
      // Calculate ETAs in real-time from status log (21-day rolling window)
      const twentyOneDaysAgo = new Date();
      twentyOneDaysAgo.setDate(twentyOneDaysAgo.getDate() - 21);
      
      // Get all status changes from the last 21 days
      const statusLogCommand = new ScanCommand({
        TableName: TABLES.ASSIGNMENT_STATUS_LOG,
        FilterExpression: 'changed_at >= :cutoffDate',
        ExpressionAttributeValues: {
          ':cutoffDate': { S: twentyOneDaysAgo.toISOString() }
        }
      });
      
      const statusLogResult = await this.client.send(statusLogCommand);
      const statusChanges = (statusLogResult.Items || []).map(item => ({
        assignment_id: item.assignment_id?.S || '',
        from_status: item.from_status?.S || '',
        to_status: item.to_status?.S || '',
        practice: item.practice?.S || '',
      ...
      // Get all assignments to find creation dates
      const assignments = await this.getAllAssignments();
      const assignmentMap = new Map(assignments.map(a => [a.id, a]));
      
      // Calculate ETAs by practice
      const practiceETAMap = new Map();
      const practices = practiceList.length > 0 ? practiceList : [...new Set(statusChanges.map(sc => sc.practice).filter(p => p && p !== 'Pending'))];
      
      for (const practice of practices) {
        // Calculate Pending -> Unassigned ETA
        const pendingToUnassigned = statusChanges.filter(sc => 
          sc.practice === practice && 
          sc.from_status === 'Pending' && 
          sc.to_status === 'Unassigned'
        );
        
        const pendingDurations = [];
        for (const change of pendingToUnassigned) {
          const assignment = assignmentMap.get(change.assignment_id);
          if (assignment && assignment.created_at) {
            ...
            if (hours > 0) pendingDurations.push(hours);
          }
        }
        
        if (pendingDurations.length > 0) {
          const avgHours = pendingDurations.reduce((sum, h) => sum + h, 0) / pendingDurations.length;
          practiceETAMap.set(`${practice}_pending_to_unassigned`, {
            practice,
            statusTransition: 'pending_to_unassigned',
            avgDurationHours: Math.round(avgHours),
            sampleSize: pendingDurations.length
          });
        }
        
        // Calculate Unassigned -> Assigned ETA
        const unassignedToAssigned = statusChanges.filter(sc => 
          sc.practice === practice && 
          sc.from_status === 'Unassigned' && 
          sc.to_status === 'Assigned'
        );
        
        const resourceDurations = [];
        for (const assignedChange of unassignedToAssigned) {
          // Find when this assignment became Unassigned
          const unassignedChange = statusChanges.find(sc => 
            sc.assignment_id === assignedChange.assignment_id &&
            sc.to_status === 'Unassigned'
          );
          
          if (unassignedChange) {
            const unassignedDate = new Date(unassignedChange.changed_at);
            const assignedDate = new Date(assignedChange.changed_at);
            const hours = (assignedDate - unassignedDate) / (1000 * 60 * 60);
            if (hours > 0) resourceDurations.push(hours);
          }
        }
        
        if (resourceDurations.length > 0) {
          const avgHours = resourceDurations.reduce((sum, h) => sum + h, 0) / resourceDurations.length;
          practiceETAMap.set(`${practice}_unassigned_to_assigned`, {
            practice,
            statusTransition: 'unassigned_to_assigned',
            avgDurationHours: Math.round(avgHours),
            sampleSize: resourceDurations.length
          });
        }
      }
      
      return Array.from(practiceETAMap.values());
    } catch (error) {
      if (error.name === 'ResourceNotFoundException') {
        return [];
      }
      logger.error('Error calculating practice ETAs:', error);
      return [];
    }
  }
  ...
  async getCompanies(practiceGroupId, contactType) {
    try {
      ...
      const result = await this.client.send(command);
      const mappedCompanies = (result.Items || []).map(item => {
  ...
  async updateCompany(id, updates, userInfo = null) {
    try {
      ...
      // Convert frontend field names to database field names and process arrays
      ...
      Object.keys(updates).forEach(key => {
        if (key !== 'id') {
          ...
          if (key === 'msaSigned') {
            dbFieldName = 'msa_signed';
          } else if (key === 'solutionType') {
            dbFieldName = 'solution_type';
            if (Array.isArray(value)) {
              value = JSON.stringify(value);
            }
          } else if (key === 'technology' && Array.isArray(value)) {
            value = JSON.stringify(value);
          }
          
          processedUpdates[dbFieldName] = value;
        }
      });
      ...
      Object.keys(updates).forEach((key) => {
        if (key !== 'id') {
          // Map frontend field names to database field names for comparison
          let currentFieldName = key;
          if (key === 'msaSigned') {
            currentFieldName = 'msa_signed';
          } else if (key === 'solutionType') {
            currentFieldName = 'solution_type';
          }
          
          let currentValue = currentCompany[currentFieldName];
      ...
      Object.keys(processedUpdates).forEach((dbFieldName) => {
        updateExpressions.push(`#${dbFieldName} = :${dbFieldName}`);
        attributeNames[`#${dbFieldName}`] = dbFieldName;
        attributeValues[`:${dbFieldName}`] = { S: processedUpdates[dbFieldName] };
      });
    ...
    try {
      ...
      Object.keys(updates).forEach(key => {
        if (key !== 'id') {
          ...
          // Handle field name mappings for phone fields
          if (key === 'cellPhone') {
            dbFieldName = 'cell_phone';
          } else if (key === 'officePhone') {
            dbFieldName = 'office_phone';
          }
      ...
      Object.keys(processedUpdates).forEach((dbFieldName) => {
        if (currentContact[dbFieldName] !== processedUpdates[dbFieldName]) {
          const fieldName = fieldMappings[dbFieldName] || dbFieldName;
          changes.push({
            field: fieldName,
            from: currentContact[dbFieldName] || '',
            to: processedUpdates[dbFieldName] || ''
          });
        }
      });
  ...
  async addTrainingCertSignUp(trainingCertId, userEmail, userName) {
    try {
      const getCommand = new GetItemCommand({
        TableName: TABLES.TRAINING_CERTS,
        Key: { id: { S: trainingCertId } }
      ...
      const signUps = result.Item.sign_ups ? JSON.parse(result.Item.sign_ups.S) : [];
      
      // Check if user already signed up
      if (signUps.some(signup => signup.email === userEmail)) {
      ...
      const updateCommand = new UpdateItemCommand({
        TableName: TABLES.TRAINING_CERTS,
        Key: { id: { S: trainingCertId } },
        UpdateExpression: 'SET sign_ups = :signUps',
        ExpressionAttributeValues: {
          ':signUps': { S: JSON.stringify(signUps) }
        }
      });
      ...
      logger.error('DynamoDB addTrainingCertSignUp error', { error: error.message });
      return false;
    }
  }
