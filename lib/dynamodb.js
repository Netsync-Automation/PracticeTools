    ...
    try {
      ...
      if (statuses.length === 0) {
        const defaultStatuses = [
          ...
          { value: 'Approved', order: 5 },
          { value: 'Complete', order: 6 }
        ];
      ...
      return ['Pending', 'Unassigned', 'Assigned', 'Pending Approval', 'Approved', 'Complete'];
    }
  }
  ...
  async addSaAssignment(practice, status, opportunityId, requestDate, eta, customerName, opportunityName, region, am, saAssigned, dateAssigned, notes, attachments = [], notificationUsers = [], scoopUrl = '', isr = '', submittedBy = '') {
    ...
    if (typeof window === 'undefined') {
      try {
        ...
        // Convert SA names to "Name <email>" format
        if (saAssigned && saAssigned.trim()) {
          const users = await this.getAllUsers();
          ...
          logger.info('Converted SA names to include emails', {
            original: saAssigned,
