...
export class DynamoDBService {
  ...
  async getTrainingCertById(id) {
    ...
    try {
      ...
      return result.Item ? this.formatTrainingCertItem(result.Item) : null;
    } catch (error) {
  ...
  async getAllTrainingCerts() {
    const command = new ScanCommand({
      TableName: TABLES.TRAINING_CERTS,
      ConsistentRead: true  // Force consistent read to avoid eventual consistency issues
    });
    
    try {
      const result = await this.client.send(command);
      const trainingCerts = (result.Items || []).map(item => this.formatTrainingCertItem(item));
      return trainingCerts.sort((a, b) => new Date(b.created_at) - new Date(a.created_at));
    } catch (error) {
      if (error.name === 'ResourceNotFoundException') {
        await this.createTrainingCertsTable();
        return [];
  ...
  async updateTrainingCert(id, updates) {
    try {
      ...
      // Process all valid update fields
      Object.keys(updates).forEach(key => {
        if (key !== 'id' && updates[key] !== undefined && updates[key] !== null) {
          if (key === 'lastEditedBy') {
            updateExpressions.push(`#last_edited_by = :last_edited_by`);
            attributeNames['#last_edited_by'] = 'last_edited_by';
            attributeValues[':last_edited_by'] = { S: String(updates[key] || '') };
  ...
  formatTrainingCertItem(item) {
    ...
    return {
      ...
      code: item.code?.S || '',
      level: item.level?.S || '',
      trainingType: item.training_type?.S || item.trainingType?.S || '',
      prerequisites: item.prerequisites?.S || '',
      examsRequired: item.exams_required?.S || item.examsRequired?.S || '',
      examCost: item.exam_cost?.S || item.examCost?.S || '',
      quantityNeeded: item.quantity_needed?.S || item.quantityNeeded?.S || '',
