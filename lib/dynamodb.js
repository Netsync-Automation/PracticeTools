...
export class DynamoDBService {
      ...
      Object.keys(updates).forEach((key) => {
        if (key !== 'id') {
          if (key === 'lastEditedBy') {
            ...
            attributeValues[':last_edited_by'] = { S: updates[key] || '' };
          } else {
  ...
  formatTrainingCertItem(item) {
    ...
    return {
      ...
      code: item.code?.S || '',
      level: item.level?.S || '',
      trainingType: item.trainingType?.S || '',
      prerequisites: item.prerequisites?.S || '',
      examsRequired: item.examsRequired?.S || '',
      examCost: item.examCost?.S || '',
      quantityNeeded: item.quantityNeeded?.S || '',
  ...
  async updateTrainingCert(id, practice, type, vendor, name, code, level, trainingType, prerequisites, examsRequired, examCost, quantityNeeded, incentive, notes, updatedBy) {
    ...
    try {
      const command = new UpdateItemCommand({
        TableName: TABLES.TRAINING_CERTS,
        Key: { id: { S: id } },
        UpdateExpression: 'SET practice = :practice, #type = :type, vendor = :vendor, #name = :name, code = :code, #level = :level, training_type = :trainingType, prerequisites = :prerequisites, exams_required = :examsRequired, exam_cost = :examCost, quantity_needed = :quantityNeeded, incentive = :incentive, notes = :notes, last_edited = :lastEdited, updated_at = :updatedAt',
        ExpressionAttributeNames: {
          '#type': 'type',
          '#name': 'name',
          '#level': 'level'
