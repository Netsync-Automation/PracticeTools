...
export class DynamoDBService {
  ...
  async deleteAssignment(id) {
    try {
      ...
      if (assignment && assignment.webex_space_id) {
        try {
          const { webexService } = await import('./webex-service.js');
          ...
          // Don't fail the deletion if Webex cleanup fails
        }
      }
  ...
  async getEmailActions() {
    try {
      ...
      if (error.name === 'ResourceNotFoundException') {
        await this.createEmailActionsTable();
        return [];
      }
    ...
    try {
      const command = new PutItemCommand({
        ...
        Item: {
          ...
          created_at: { S: action.created_at },
          environment: { S: action.environment }
        }
      });
      ...
      if (error.name === 'ResourceNotFoundException') {
        await this.createEmailActionsTable();
        return this.createEmailAction(action);
      }
