...
export class DynamoDBService {
  ...
  async addAssignment(practice, status, projectNumber, requestDate, eta, customerName, projectDescription, region, am, pm, resourceAssigned, dateAssigned, notes, documentationLink = '', pmEmail = '', attachments = [], notificationUsers = []) {
    ...
    // Process user fields to store both names and emails (DSR compliance)
    let amEmail = '', resourceAssignedEmail = '';
    ...
    if (typeof window === 'undefined') {
      try {
        ...
        // Get Resource Assigned email
        if (resourceAssigned) {
          const resourceUser = users.find(user => user.name.toLowerCase() === resourceAssigned.toLowerCase());
          if (resourceUser) {
            resourceAssignedEmail = resourceUser.email;
          }
        }
        
        logger.info('Processed user emails for resource assignment', {
          am: am,
          amEmail: amEmail,
          resourceAssigned: resourceAssigned,
          resourceAssignedEmail: resourceAssignedEmail
        });
      } catch (error) {
        logger.error('Error processing user emails in addAssignment', { error: error.message });
      }
    }
    ...
    const command = new PutItemCommand({
      ...
      Item: {
        ...
        resource_assigned_email: { S: resourceAssignedEmail },
  ...
  async updateAssignment(id, updates) {
    try {
      ...
      // Process user field updates to maintain email consistency (DSR compliance)
      if (typeof window === 'undefined') {
        try {
          const users = await this.getAllUsers();
          
          // Update AM email if AM name changed
          if (updates.am !== undefined) {
            const amUser = users.find(user => user.name.toLowerCase() === (updates.am || '').toLowerCase());
            updates.am_email = amUser ? amUser.email : '';
          }
          
          // Update Resource Assigned email if name changed
          if (updates.resourceAssigned !== undefined) {
            const resourceUser = users.find(user => user.name.toLowerCase() === (updates.resourceAssigned || '').toLowerCase());
            updates.resource_assigned_email = resourceUser ? resourceUser.email : '';
          }
          
          logger.info('Updated user emails for assignment update', {
            assignmentId: id,
            amUpdated: updates.am !== undefined,
            resourceAssignedUpdated: updates.resourceAssigned !== undefined
          });
        } catch (error) {
          logger.error('Error updating user emails in updateAssignment', { error: error.message });
        }
      }
  ...
  async addSaAssignment(practice, status, opportunityId, requestDate, eta, customerName, opportunityName, region, am, saAssigned, dateAssigned, notes, attachments = [], notificationUsers = [], scoopUrl = '', isr = '', submittedBy = '') {
    const id = uuidv4();
  ...
  async getAllPracticeBoards() {
    try {
      ...
      return (result.Items || []).map(item => {
        ...
        const practiceId = item.setting_key?.S?.replace('practice_board_', '') || '';
