...
export class EmailProcessor {
  ...
  async executeSaAssignmentAction(email, rule) {
    try {
      ...
      // If no To: users found, try to extract from email content directly
      if (toUsers.length === 0) {
        ...
        if (toMatch) {
          ...
          // Convert names to user objects with email lookup
          const users = await db.getAllUsers();
          toUsers = names.map(name => {
            const user = users.find(u => u.name.toLowerCase().includes(name.toLowerCase()) || name.toLowerCase().includes(u.name.toLowerCase()));
            return user ? { name: user.name, email: user.email } : { name: name, email: '' };
          }).filter(user => user.email); // Only include users with emails
          
          logger.info('Extracted To: users from email content', {
            originalToLine: toLine,
            extractedUsers: toUsers
          });
        }
      }
      ...
      logger.info('DSR: Skipping automatic AM/ISR user creation', {
        am: extractedData.am,
        isr: extractedData.isr
      });
      ...
      const saAssignmentId = await db.addSaAssignment(
        ...
        extractedData.notes || '', // notes
        [], // attachments
        extractedData.saAssignmentNotificationUsers || [], // DSR: notification users from CC field only
    ...
    if (toLine) {
      ...
      recipients.forEach((recipient, index) => {
        ...
        if (cleanRecipient) {
          ...
          if (angleMatch) {
            name = angleMatch[1].trim().replace(/^To:\s*/i, '').replace(/^To\s+/i, ''); // Remove "To: " and "To " prefixes
            ...
            if (emailRegex.test(cleanEmail)) {
              ...
              // This is just a name, not an email - skip it
              logger.info(`Skipping non-email recipient`, { recipient: cleanRecipient });
            }
          }
