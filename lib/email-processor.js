...
import { EMAIL_FIELD_MAPPINGS } from '../constants/email-keyword-fields.js';
...
export class EmailProcessor {
  ...
  async processEmail(email, rules) {
    try {
      ...
      // Create resource assignment using constants
      const resourceAssignment = {};
      
      // Map extracted data using field mappings constants
      EMAIL_FIELD_MAPPINGS.forEach(field => {
      ...
      if (assignmentId) {
        ...
        try {
          ...
          logger.info('SSE notification sent for new assignment', { assignmentId });
        } catch (sseError) {
          logger.error('Failed to send SSE notification', { error: sseError.message });
        }
