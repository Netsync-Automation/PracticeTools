...
import { processAmIsrUsers } from './user-manager.js';
...
export class EmailProcessor {
  ...
  async executeSaAssignmentApprovalRequestAction(email, rule) {
    try {
      ...
      try {
        ...
        logger.info('Email processor API call debug', {
          env,
          baseUrl,
        ...
        if (response.ok) {
          ...
          // If we found the practice, update all other SAs in the same practice to Pending Approval
          if (targetPractice && matchingAssignment.practiceAssignments) {
            try {
              const practiceAssignments = JSON.parse(matchingAssignment.practiceAssignments);
              const practicesSAs = practiceAssignments[targetPractice] || [];
              
              // Update other SAs in the practice via API calls
              for (const practicesSA of practicesSAs) {
                if (practicesSA !== saKey) {
                  ...
                  if (!otherSAResponse.ok) {
                    logger.error('Failed to update practice SA status via API', {
                      saAssignmentId: matchingAssignment.id,
                      targetSA: practicesSA,
              ...
              logger.info('Updated all SAs in practice to Pending Approval via API', {
                targetPractice,
                practicesSAs,
                updatedSACount: practicesSAs.length,
                opportunityId
              });
            } catch (e) {
  ...
  async executeSaAssignmentAction(email, rule) {
    try {
      ...
      // Special handling for SA assignments: map To: users to AM, ISR, and SA fields
      if (toUsers && toUsers.length > 0) {
        ...
        // Do NOT map To: users as SA assignments - they are AM/ISR, not SAs
        // SA assignments will be handled by auto-assignment based on mapping database
        logger.info('To: users mapped to AM/ISR only, not SA assignments', {
          userCount: toUsers.length,
          amUser: toUsers[0]?.name,
          isrUser: toUsers[1]?.name
        });
      }
      ...
      const matchedPractice = await extractAndMatchPractice(email.body);
      extractedData.practice = matchedPractice;
      ...
      // Use AM region if available from existing user
      if (amIsrResult.amRegion) {
        extractedData.region = amIsrResult.amRegion;
        logger.info('Using AM region from existing user', {
          region: amIsrResult.amRegion,
          amEmail: amIsrResult.amUser?.email
        });
      }
      ...
      logger.info('About to call db.addSaAssignment with parameters', {
        ...
        amUserCreated: !!amIsrResult.amUser && amIsrResult.amUser.created_from === 'SYSTEM',
        isrUserCreated: !!amIsrResult.isrUser && amIsrResult.isrUser.created_from === 'SYSTEM'
      });
      ...
      const saAssignmentId = await db.addSaAssignment(
        ...
        (extractedData.region || '').toUpperCase(), // region - convert to uppercase
        extractedData.am || '', // am
        '', // saAssigned - leave empty, will be populated by auto-assignment
        extractedData.saAssigned ? new Date().toISOString().split('T')[0] : '', // dateAssigned - set if SAs assigned
        extractedData.notes || '', // notes
        [], // attachments
...
${email.body}`.toLowerCase();
      ...
      if (keywordIndex !== -1) {
        ...
        if (keyword === 'to:') {
          ...
          if (forwardedToMatch) {
            ...
            if (toUsers.length > 0) {
              // Set resource assignment notification users
              extractedData.resourceAssignmentNotificationUsers = toUsers;
              // Only set SA assignment notification users if not already set by CC field
              if (!extractedData.saAssignmentNotificationUsers || (Array.isArray(extractedData.saAssignmentNotificationUsers) && extractedData.saAssignmentNotificationUsers.length === 0)) {
                extractedData.saAssignmentNotificationUsers = toUsers;
                logger.info('Setting SA notification users from To: field (no CC users found)', {
                  toUsers: toUsers,
                  userCount: toUsers.length
                });
              } else {
                logger.info('Preserving SA notification users from CC field (not overwriting with To: users)', {
                  ccUsers: extractedData.saAssignmentNotificationUsers,
                  ccUserCount: extractedData.saAssignmentNotificationUsers.length,
                  toUsers: toUsers,
                  toUserCount: toUsers.length
                });
              }
              // Don't overwrite CC users with To: users
              ...
              logger.info('Extracted To: users', { 
                ...
                overwritingSaNotifications: !extractedData.saAssignmentNotificationUsers
              });
            }
          }
          continue;
        ...
        if (extractedValue) {
          ...
          if (extractedValue) {
            ...
            // Special handling for CC field to parse users like To: field
            if (mapping.field === 'sa_assignment_notification_users') {
              const ccUsers = this.parseToLine(extractedValue);
              ...
              logger.info('Extracted and parsed CC users', { 
                keyword: mapping.keyword,
                field: mapping.field,
    ...
    // Final check: restore CC users if they were overwritten
    if (ccUsersBackup && ccUsersBackup.length > 0) {
      extractedData.saAssignmentNotificationUsers = ccUsersBackup;
      logger.info('Final SA notification users check - restored CC users', {
    ...
    if (toLine) {
      ...
      recipients.forEach((recipient, index) => {
        ...
        if (cleanRecipient) {
          ...
          if (angleMatch) {
            name = angleMatch[1].trim().replace(/^To:\s*/i, ''); // Remove "To: " prefix
