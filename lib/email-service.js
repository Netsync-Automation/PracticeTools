    ...
    try {
      ...
      for (const practice of PRACTICE_OPTIONS.filter(p => p !== 'Pending')) {
        ...
        if (eta && eta.practice_assignment_eta_hours > 0) {
          const days = eta.practice_assignment_eta_hours / 24; // Convert to days
    ...
    try {
      ...
      for (const practice of assignedPractices) {
        ...
        if (eta && eta.resource_assignment_eta_hours >= 0) {
          const days = eta.resource_assignment_eta_hours / 24; // Convert to days
          const roundedDays = Math.round(days * 100) / 100; // Round to 2 decimal places
          practiceETAs[practice] = roundedDays.toFixed(2);
        }
      }
  ...
  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f5f7fa; padding: 20px 0; mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
    <tr>
      ...
      if (assignment.resource_assigned_email) {
        ...
        logger.info('Added resource assigned email to recipients', { resourceEmail: assignment.resource_assigned_email });
      }
      ...
      // Add assigned resources (get their emails from users table as fallback)
      const users = await db.getAllUsers();
      assignedResources.forEach(resourceName => {
        ...
        if (user && user.email && !recipients.includes(user.email)) {
          recipients.push(user.email);
          logger.info('Added resource email to recipients (fallback)', { userEmail: user.email, userName: user.name });
        }
      });
