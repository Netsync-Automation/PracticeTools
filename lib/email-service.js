...
class EmailService {
  ...
  // DSR: Format SMTP username as email address for EWS compatibility
  formatEmailAddress(smtpUser) {
    if (!smtpUser) return smtpUser;
    // If already has @, return as-is
    if (smtpUser.includes('@')) return smtpUser;
    // Append @netsync.com for EWS authentication compatibility
    return `${smtpUser}@netsync.com`;
  }
  ...
  async sendPendingAssignmentNotification(assignment) {
    try {
      ...
      // Get all practice ETAs directly from database
      const practiceETAs = {};
      const allETAs = await db.getPracticeETAs();
      
      for (const practice of PRACTICE_OPTIONS.filter(p => p !== 'Pending')) {
        const practiceETA = allETAs.find(eta => 
          eta.practice === practice && 
          eta.statusTransition === 'pending_to_unassigned' && 
          eta.avgDurationHours > 0
        );
        
        if (practiceETA) {
          const days = practiceETA.avgDurationHours / 24;
...
<body style="margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f5f7fa; line-height: 1.4; -webkit-text-size-adjust: 100%; -ms-text-size-adjust: 100%;">
  <table border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f5f7fa; padding: 20px 0; mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
    <tr>
      ...
      const fromEmail = this.formatEmailAddress(smtpUser || process.env.SMTP_USERNAME);
  ...
  async sendPracticeAssignedNotification(assignment) {
    try {
      ...
      // Get practice ETAs for assigned practices directly from database
      const assignedPractices = assignment.practice ? assignment.practice.split(',').map(p => p.trim()) : [];
      const practiceETAs = {};
      const allETAs = await db.getPracticeETAs(assignedPractices);
      
      for (const practice of assignedPractices) {
        const resourceETA = allETAs.find(eta => 
          eta.practice === practice && 
          eta.statusTransition === 'unassigned_to_assigned' && 
          eta.avgDurationHours > 0
        );
        
        if (resourceETA) {
          const days = resourceETA.avgDurationHours / 24;
          const roundedDays = Math.round(days * 100) / 100;
          practiceETAs[practice] = roundedDays.toFixed(2);
        }
      }
      ...
      // Get SMTP settings
      const [smtpHost, smtpPort, smtpUser] = await Promise.all([
        getSecureParameter(`${ssmPrefix}/SMTP_HOST`),
        getSecureParameter(`${ssmPrefix}/SMTP_PORT`),
        getSecureParameter(`${ssmPrefix}/SMTP_USERNAME`)
      ]);
