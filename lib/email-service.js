  ...
  async sendPendingAssignmentNotification(assignment) {
    try {
      ...
      for (const practice of PRACTICE_OPTIONS.filter(p => p !== 'Pending')) {
        try {
          // Get ETA data directly from database
          const { ScanCommand } = await import('@aws-sdk/client-dynamodb');
          const { getTableName } = await import('./dynamodb.js');
          
          const command = new ScanCommand({
            TableName: getTableName('PracticeETAs'),
            FilterExpression: 'practice = :practice AND status_transition = :transition',
            ExpressionAttributeValues: {
              ':practice': { S: practice },
              ':transition': { S: 'pending_to_unassigned' }
            }
          });
          
          const result = await db.client.send(command);
          if (result.Items && result.Items.length > 0) {
            const eta = result.Items[0];
            const avgHours = parseFloat(eta.avg_duration_hours?.N || '0');
            if (avgHours > 0) {
      ...
      for (const practice of assignedPractices) {
        try {
          ...
          const command = new ScanCommand({
            ...
            ExpressionAttributeValues: {
              ':practice': { S: practice },
              ':transition': { S: 'unassigned_to_assigned' }
            }
          });
