...
export class ETATracker {
  ...
  async processStatusTransition(assignment, practice, oldStatus, newStatus, changeTime) {
    ...
    // Calculate duration from assignment creation or last status change
    const duration = this.calculateDuration(assignment, oldStatus, changeTime);
    if (duration <= 0) return;
    
    const durationHours = duration / (1000 * 60 * 60); // Convert to hours
    
    // Determine if this is SA-specific tracking (for Assigned -> Completed)
    const saName = transitionType === 'assigned_to_completed' ? assignment.assignedSA : null;
    
    // Update ETA database
    await this.updateETADatabase(practice, saName, transitionType, durationHours);
  ...
  getTransitionType(oldStatus, newStatus) {
    ...
    if ((oldStatus === 'Pending' || oldStatus === 'Unassigned') && newStatus === 'Assigned') {
      return 'to_assigned';
    }
  ...
  calculateDuration(assignment, oldStatus, changeTime) {
    ...
    if (oldStatus === 'Pending') {
      startTime = new Date(assignment.createdAt || assignment.created_at);
