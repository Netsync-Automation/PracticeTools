...
import { getSSMParameter } from './ssm.js';
...
class EWSClient {
  ...
  async checkNewMail(sinceDateTime = null) {
    if (!this.isConnected) {
      ...
      if (!initialized) {
        throw new Error('Failed to initialize EWS connection');
      }
    }
    ...
    try {
      ...
      let searchFilter = new SearchFilter.IsEqualTo(
        'IsRead', 
        false
      );
      ...
      // Create item view (limit to 50 emails)
      const itemView = new ItemView(50);
      itemView.PropertySet = new PropertySet(
        BasePropertySet.FirstClassProperties,
        ['Subject', 'From', 'DateTimeReceived', 'Body', 'IsRead', 'HasAttachments']
      );

      // Find items
      const findResults = await this.service.FindItems(inbox.Id, searchFilter, itemView);
      ...
      for (const item of findResults.Items) {
        emails.push({
          ...
          from: item.From ? item.From.Address : 'Unknown',
          fromName: item.From ? item.From.Name : 'Unknown',
          dateReceived: item.DateTimeReceived,
          body: item.Body ? item.Body.Text : '',
          isRead: item.IsRead,
      ...
      logger.info(`EWS: Found ${emails.length} new emails`);
      return emails;
  ...
  async markAsRead(emailId) {
    if (!this.isConnected) {
      throw new Error('EWS client not connected');
    }

    try {
      const item = await this.service.BindToItem(emailId, new PropertySet(['IsRead']));
      item.IsRead = true;
      await item.Update();
      logger.info(`EWS: Marked email ${emailId} as read`);
      return true;
    } catch (error) {
