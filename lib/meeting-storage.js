...
async function storeMeetingData(meetingData) {
  ...
  const item = {
    ...
    s3Key: meetingData.s3Key, // S3 key for downloaded MP4
    transcript: {
      text: meetingData.transcript,
      chunks: chunkTranscript(meetingData.transcript), // For AI processing
      metadata: {
        wordCount: meetingData.transcript ? meetingData.transcript.split(' ').length : 0,
...
async function findMeetingByInstanceId(endedInstanceId) {
  await createWebexMeetingsTable();
  const tableName = getTableName('webex_meetings');
  ...
  try {
    const result = await docClient.send(new GetCommand({
      TableName: tableName,
      Key: { meetingId: endedInstanceId }
    }));
    if (result.Item) return result.Item;
  } catch (error) {
    console.log('Exact match failed, trying scan');
  }
  
  // Fallback: scan for meetingInstanceId match
  const scanResult = await docClient.send(new ScanCommand({
    TableName: tableName,
    FilterExpression: 'meetingInstanceId = :instanceId AND #env = :env',
    ExpressionAttributeNames: { '#env': 'environment' },
    ExpressionAttributeValues: {
      ':instanceId': endedInstanceId,
      ':env': getEnvironment()
    }
  }));
...
async function updateMeetingTranscript(meetingId, transcript) {
  ...
  await docClient.send(new UpdateCommand({
    TableName: tableName,
    Key: { meetingId },
    UpdateExpression: 'SET transcript = :transcript, updatedAt = :updatedAt',
    ExpressionAttributeValues: {
...
export { storeMeetingData, getMeetings, updateMeetingTranscript, findMeetingByInstanceId };