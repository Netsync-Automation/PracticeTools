...
export class NTLMAuthenticator {
  ...
  async makeAuthenticatedRequest(path, soapBody, testUsername = null) {
    return new Promise((resolve, reject) => {
      const useUsername = testUsername || this.username;
      const options = {
  ...
  async testConnection() {
    logger.info('NTLM: Test connection - credentials check', {
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:t="http://schemas.microsoft.com/exchange/services/2006/types">
  ...
  <soap:Body>
    ...
    const usernameVariations = [
      ...
      `${this.domain.toUpperCase()}\\${this.username}`,
      `${this.username}@${this.domain}`,
      `${this.username}@netsyncnetwork.com`
    ];

    for (const usernameVariant of usernameVariations) {
      try {
        logger.info('NTLM: Trying username variant', { username: usernameVariant });
        const result = await this.makeAuthenticatedRequest('/EWS/Exchange.asmx', soapBody, usernameVariant);
        
        if (result.statusCode === 200) {
          logger.info('NTLM: Authentication successful', { workingUsername: usernameVariant });
          return {
            success: true,
            message: `NTLM authentication successful with username: ${usernameVariant}`,
            statusCode: result.statusCode,
            workingUsername: usernameVariant
          };
        } else {
          logger.info('NTLM: Username variant failed', { username: usernameVariant, statusCode: result.statusCode });
        }
      } catch (error) {
        logger.info('NTLM: Username variant error', { username: usernameVariant, error: error.message });
      }
    }

    return {
      success: false,
      error: 'NTLM authentication failed - either wrong credentials or httpntlm library issue',
      testedUsernames: usernameVariations,
      suggestion: 'Verify password in SSM is correct. The httpntlm library may not support this Exchange server NTLM version.'
    };
  }
