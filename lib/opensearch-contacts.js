  ...
  try {
    await client.index({
      ...
      body: {
        ...company,
        practiceGroupName,
        contactType: company.contactType || company.contact_type,
        deleted: false
      }
    });
  } catch (error) {
...
export const deleteCompanyFromIndex = async (companyId) => {
  ...
  try {
    await client.deleteByQuery({
      ...
      body: {
        query: {
          term: { 'id.keyword': companyId }
        }
      }
    });
  } catch (error) {
    console.error('Error deleting company from index:', error);
  }
};
...
export const deleteContactFromIndex = async (contactId) => {
  ...
  try {
    await client.deleteByQuery({
      ...
      body: {
        query: {
          term: { 'id.keyword': contactId }
        }
      }
    });
  } catch (error) {
    console.error('Error deleting contact from index:', error);
  }
};
