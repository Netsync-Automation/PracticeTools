...
export class SaAutoAssignment {
  ...
  async processAutoAssignment(saAssignmentId) {
    try {
      ...
      // Combine existing SAs with new ones, avoiding duplicates by email
      const existingSas = saAssignment.saAssigned ? saAssignment.saAssigned.split(',').map(s => s.trim()) : [];
      const existingSaEmails = existingSas.map(sa => {
        const match = sa.match(/<([^>]+)>/);
        return match ? match[1] : null;
      }).filter(Boolean);
      
      const newSasToAdd = uniqueSas.filter(saName => {
        const user = users.find(u => u.name === saName);
        return user && !existingSaEmails.includes(user.email);
      });
      const combinedSas = [...existingSas, ...newSasToAdd];
  ...
  async updateSaAssignment(saAssignmentId, assignedSas, region, practices, matchingSas, existingSas = []) {
    try {
      // Get SA assignment to check AM email
      const saAssignment = await db.getSaAssignmentById(saAssignmentId);
      if (!saAssignment) {
        return { success: false, error: 'SA assignment not found' };
      }
      
      // Extract AM email from SA assignment
      const extractEmail = (nameWithEmail) => {
        if (!nameWithEmail) return null;
        const match = nameWithEmail.match(/<([^>]+)>/);
        return match ? match[1] : nameWithEmail.toLowerCase().includes('@') ? nameWithEmail : null;
      ...
      let amEmail = extractEmail(saAssignment.am);
      if (!amEmail && saAssignment.am) {
        const users = await db.getAllUsers();
        const user = users.find(u => u.name === saAssignment.am);
        if (user && user.email) {
          amEmail = user.email;
        }
      }
      ...
      if (!amEmail) {
        logger.error('Could not determine AM email for SA assignment', { saAssignmentId, am: saAssignment.am });
        return { success: false, error: 'Could not determine AM email' };
      }
      
      // Build practice-specific SA assignments
      const users = await db.getAllUsers();
      const practiceAssignments = {};
      const practicesWithSas = new Set();
      
      // Initialize practice assignments
      practices.forEach(practice => {
        practiceAssignments[practice] = [];
      });
      
      // Process each unique mapping (not each assigned SA)
      const processedSas = new Set(); // Track processed SAs to avoid duplicates
      
      matchingSas.forEach(mapping => {
        const user = users.find(u => u.name === mapping.saName);
        if (!user) return;
        
        // Convert SA name to "Name <email>" format
        const saWithEmail = `${user.name} <${user.email}>`;
        
        // Assign SA to specific practices they have mappings for
        if (mapping.practices) {
          mapping.practices.forEach(practice => {
            if (practices.includes(practice)) {
              // Create unique key for SA + practice combination
              const saKey = `${user.email}:${practice}`;
              
              if (!processedSas.has(saKey)) {
                practiceAssignments[practice].push(saWithEmail);
                practicesWithSas.add(practice);
                processedSas.add(saKey);
                
                logger.info('SA assigned to specific practice', {
                  saName: mapping.saName,
                  practice,
                  amEmail
                });
      ...
      const updates = {
        practiceAssignments: JSON.stringify(practiceAssignments),
        region: region,
        status: finalStatus,
        dateAssigned: allPracticesCovered ? new Date().toISOString().split('T')[0] : ''
      };
      
      logger.info('Updating SA assignment with practice-specific assignments', {
        saAssignmentId,
        amEmail,
        practices,
