...
class SAWebexService {
  ...
  async sendStatusChangeNotification(spaceId, saAssignment, accessToken, changedBy, oldStatus, newStatus, isOverallStatus = false) {
    try {
      const statusEmojis = {
        'In Progress': '🔄',
        'Pending Approval': '⏳',
      ...
      const title = isOverallStatus ? 
        `📊 SA Assignment Status Updated` : 
        `👤 Individual SA Status Updated`;
      
      const subtitle = isOverallStatus ?
        `Overall assignment status changed` :
        `${changedBy} status changed`;
        ...
        body: [
          ...
          },
          {
            ...
            items: [
              {
                type: "TextBlock",
                text: `🔄 Status transition: **${oldStatus}** → **${newStatus}**`,
                wrap: true,
                size: "Small",
                weight: "Bolder"
              }
