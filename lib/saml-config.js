...
async function getSSMParameter(paramName, envVarName) {
  ...
  // Add small delay to allow App Runner env var injection to complete
  await new Promise(resolve => setTimeout(resolve, 100));
  
  // Check environment variable again after delay
  const delayedEnvValue = process.env[envVarName];
  if (delayedEnvValue && delayedEnvValue.trim() !== '') {
    return delayedEnvValue;
  }
