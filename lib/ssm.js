...
export async function getWebexTokens(siteUrl) {
  ...
  console.log(`[SSM-DEBUG] getWebexTokens: siteUrl=${siteUrl} sitePrefix=${sitePrefix} env=${environment}`);
  console.log(`[SSM-DEBUG] Fetching from SSM: ${accessTokenPath}`);
  console.log(`[SSM-DEBUG] Fetching from SSM: ${refreshTokenPath}`);
  ...
  try {
    ...
    console.log(`[SSM-DEBUG] SSM fetch successful: accessToken.length=${accessTokenResult.Parameter.Value?.length}`);
