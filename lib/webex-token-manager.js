...
async function isTokenExpired(token) {
  ...
  // For non-JWT tokens, test with API call
  try {
...
export async function getValidAccessToken(siteUrl) {
  console.error(`[TOKEN-MGR-DEBUG] getValidAccessToken called for siteUrl=${siteUrl}`);
  const cacheKey = siteUrl;
  const cached = tokenCache.get(cacheKey);
  
  // Check cache first (valid for 1 minute)
  if (cached && (Date.now() - cached.lastUpdated) < 60000) {
    console.error(`[TOKEN-MGR-DEBUG] Cache hit for ${siteUrl}, age=${Date.now() - cached.lastUpdated}ms`);
    if (!await isTokenExpired(cached.accessToken)) {
      console.error(`[TOKEN-MGR-DEBUG] Cached token valid, returning`);
      return cached.accessToken;
    }
    console.error(`[TOKEN-MGR-DEBUG] Cached token expired`);
  } else {
    console.error(`[TOKEN-MGR-DEBUG] Cache miss for ${siteUrl}`);
  }
  
  // Load tokens from SSM
  console.error(`[TOKEN-MGR-DEBUG] Loading tokens from SSM for ${siteUrl}`);
  const tokens = await getWebexTokens(siteUrl);
  if (!tokens) {
    console.error(`[TOKEN-MGR-DEBUG] No tokens found in SSM for ${siteUrl}`);
    throw new Error(`No tokens found for ${siteUrl}`);
  }
  console.error(`[TOKEN-MGR-DEBUG] Tokens loaded from SSM: accessToken.length=${tokens.accessToken?.length} first50=${tokens.accessToken?.substring(0, 50)}`);
  
  // Check if access token needs refresh
  console.error(`[TOKEN-MGR-DEBUG] Checking if token expired`);
  const tokenExpired = await isTokenExpired(tokens.accessToken);
  console.error(`[TOKEN-MGR-DEBUG] Token expired check result: ${tokenExpired}`);
  if (tokenExpired) {
    console.error(`[TOKEN-MGR-DEBUG] Access token expired for ${siteUrl}, refreshing...`);
    const refreshed = await refreshAccessToken(siteUrl, tokens.refreshToken);
    console.error(`[TOKEN-MGR-DEBUG] Token refreshed successfully`);
    return refreshed.accessToken;
  }
  
  // Update cache with current tokens
  console.error(`[TOKEN-MGR-DEBUG] Caching token for ${siteUrl}`);
  ...
  console.error(`[TOKEN-MGR-DEBUG] Returning token from SSM`);
